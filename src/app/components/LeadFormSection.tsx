...
export function LeadFormSection() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const API_URL = 'https://script.google.com/macros/s/AKfycbxKdQVOQhilpLYQsLavNSExep2vrg5GtLaTNH2rsJ8JDXsFK3tqTPLSNkCF1g_3Qkm1TA/exec';
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-[#2F2F2F]">املأ النموذج وسيتواصل معك أحد مهندسينا لمناقشة مشروعك</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="bg-[#E6E6E6] p-8 rounded-lg shadow-lg space-y-6">
            {/* الاسم */}
            <div className="text-right">
              <label htmlFor="name" className="block mb-2 text-[#0B1C2D]">
                الاسم <span className="text-[#FF8C00]">*</span>
            ...
            <div className="text-right">
              ...
              <input
                ...
                {...register('phone', {
                  required: 'رقم الهاتف مطلوب',
                  pattern: { value: /^[0-9+\-\s()]+$/, message: 'الرجاء إدخال رقم هاتف صحيح' },
                })}
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#FF8C00] text-right"
                placeholder="05xxxxxxxx"
                dir="ltr"
            ...
            {/* اسم المصنع */}
            <div className="text-right">
              <label htmlFor="companyName" className="block mb-2 text-[#0B1C2D]">اسم المصنع <span className="text-[#2F2F2F]/60">(اختياري)</span></label>
              <input
                id="companyName"
                type="text"
                {...register('companyName')}
            ...
            {/* الاحتياجات */}
            <div className="text-right">
              <label htmlFor="requirements" className="block mb-2 text-[#0B1C2D]">اي الاحتياج المعدني الي محتاج تنفذه؟ <span className="text-[#FF8C00]">*</span></label>
              <textarea
                id="requirements"
                {...register('requirements', { required: 'يرجى تحديد الاحتياج المعدني' })}
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#FF8C00] text-right min-h-[100px]"
            ...
            {/* الرسومات */}
            <div className="text-right">
              <label htmlFor="hasDrawings" className="block mb-2 text-[#0B1C2D]">هل في رسومات معدنية ولا لا؟ <span className="text-[#FF8C00]">*</span></label>
              <select
                id="hasDrawings"
                {...register('hasDrawings', { required: 'يرجى تحديد ما إذا كان لديك رسومات' })}
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#FF8C00] text-right"
              ...
              {errors.hasDrawings && <p className="text-red-600 text-sm mt-1">{errors.hasDrawings.message}</p>}
            </div>

            {/* نوع المشروع */}
            <div className="text-right">
              <label htmlFor="projectType" className="block mb-2 text-[#0B1C2D]">نوع المشروع <span className="text-[#2F2F2F]/60">(اختياري)</span></label>
              <select
                id="projectType"
                {...register('projectType')}
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#FF8C00] text-right"
